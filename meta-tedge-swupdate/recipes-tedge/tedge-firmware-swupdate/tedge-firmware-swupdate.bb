...
SRC_URI += " \
    file://swupdate_workflow.sh \ 
    file://firmware_update.swupdate.toml \
    file://tedge-firmware-swupdate \
    file://persist.conf \
"
...
do_install () {
    ...
    install -d "${D}${bindir}"
    install -m 0755 "${WORKDIR}/swupdate_workflow.sh" "${D}${bindir}"

    install -d "${D}${datadir}/tedge-workflows"
    install -d "${D}${sysconfdir}/tedge/operations"
    install -m 0644 "${WORKDIR}/firmware_update.swupdate.toml" "${D}${datadir}/tedge-workflows/"

    # Use a symlink to allow updating the workflow across updates
    ln --relative -s "${D}${datadir}/tedge-workflows/firmware_update.swupdate.toml" "${D}${sysconfdir}/tedge/operations/firmware_update.toml"

    # Allow sudo access
    install -d -m 0750 "${D}/etc/sudoers.d"
    install -m 0644 "${WORKDIR}/tedge-firmware-swupdate" "${D}${sysconfdir}/sudoers.d/"
...
FILES:${PN} += " \
    ${bindir}/swupdate_workflow.sh \
    ${sysconfdir}/tedge/operations/firmware_update.toml \
    ${datadir}/tedge-workflows/firmware_update.swupdate.toml \
    ${sysconfdir}/sudoers.d/tedge-firmware-swupdate \
