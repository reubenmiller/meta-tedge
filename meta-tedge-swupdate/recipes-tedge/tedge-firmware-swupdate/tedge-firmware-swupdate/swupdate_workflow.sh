...
install() {
    url="$1"
    log "Executing: swupdate install '$url'"
    EXIT_CODE="$OK"

    # TODO: Get bundle info, and check if the image requires a roboot or not
    # For now assume it does need a reboot
    IS_ROOT_FS_IMAGE=true

    if [ -f "$url" ]; then
        # TODO: Call file variant of swupdate
        $SUDO swupdate -d "-u $url" || EXIT_CODE="$?"
    else
        $SUDO swupdate -d "-u $url" || EXIT_CODE="$?"
    fi
...
commit() {
    log "Executing: committing swupdate image"
    EXIT_CODE="$OK"

    upgrade_available=$(fw_printenv upgrade_available)
    bootcount=$(fw_printenv bootcount)
    log "Debug: upgrade_available=$upgrade_available, bootcount=$bootcount"

    # TODO: How to detect the active partition when using SWUpdate?
    current_partition=$(get_current_partition)

    if [ "$current_partition" != "$EXPECTED_PARTITION" ]; then
        # Verify that the correct partition is activated
        EXIT_CODE=2
    else
        # check upgrade_available boot variable
        case "$upgrade_available" in
            1)
                log "upgrade_available is 1, clearing upgrade_available and setting bootcount back to 0"
                fw_setenv upgrade_available || EXIT_CODE=$?
                fw_setenv bootcount 0 || EXIT_CODE=$?
                ;;
            
            0)
                log "upgrade_available is 0, this is unexpected. An automatic rollback may have occurred"
                EXIT_CODE=2
                ;;
            *)
                # an empty value is similar to a 0 value, but having a different log message would help debugging
                log "upgrade_available is not set, this is unexpected. An automatic rollback may have occurred"
                EXIT_CODE=2
                ;;
        esac
    fi

    # Set logging and failure reasons (if necessary)
    case "$EXIT_CODE" in
        0)
        ...
        2)
            ...
            ;;
        *)
            log "swupdate returned code: $EXIT_CODE. Rolling back to previous partition"
...
rollback() {
    ...
    # TODO: Check if there are any additional steps required to do any rollback logic before
    # proceeding to the next state which should be rollback_reboot
    exit "$OK"
}
